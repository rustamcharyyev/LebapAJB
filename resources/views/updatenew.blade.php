        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('update-submit', $data->id) }}" method="POST" class="p-3 bg-white" enctype="multipart/form-data">
                        @csrf
                        <h1 style="font-size: 32px;" class="mb-3">Täzelik Üýtget</h1>

                        <div class="mb-3">
                            <label for="name1" class="form-label">Täzeligiň doly ady(Türkmen dili):</label>
                            <input type="text" name="name1" id="name1" class="form-control" placeholder="Ady..." value="{{ $data->name }}">
                        </div>

                        <div class="mb-3">
                            <label for="name2" class="form-label">Täzeligiň doly ady(Rus dili):</label>
                            <input type="text" name="name2" id="name2" class="form-control" placeholder="Ady..." value="{{ $data->name_ru }}">
                        </div>

                        <div class="mb-3">
                            <label for="name3" class="form-label">Täzeligiň doly ady(Iňlis dili):</label>
                            <input type="text" name="name3" id="name3" class="form-control" placeholder="Ady..." value="{{ $data->name_en }}">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="message1" id="message1" class="form-control" placeholder="Beýany...">{{ $data->content }}</textarea>
                        </div>

                        <div class="mb-3">
                            <label for="message2" class="form-label">Täzeligiň beýany (Rus dili):</label>
                            <textarea name="message2" id="message2" class="form-control" placeholder="Beýany...">{{ $data->content_ru }}</textarea>
                        </div>

                        <div class="mb-3">
                            <label for="message3" class="form-label">Täzeligiň beýany (Iňlis dili):</label>
                            <textarea name="message3" id="message3" class="form-control" placeholder="Beýany...">{{ $data->content_en }}</textarea>
                        </div>

                        <div class="field_wrapper">
                            <div>
                                <label class="form-label">Suratlary täzden ugradyň</label>
                            </div>
                            <div>
